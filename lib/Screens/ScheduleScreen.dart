...
class _ScheduleScreenState extends State<ScheduleScreen> {
  ...
  Future<void> fetchPlanById(String? planId) async {
    ...
    try {
      DocumentSnapshot planDoc = await FirebaseFirestore.instance.collection(
          'plans').doc(planId).get();
  ...
  Future<void> fetchPlansFromFirebase() async {
    try {
      CollectionReference plansRef = FirebaseFirestore.instance.collection(
          'plans');
      ...
      List<Map<String, dynamic>> fetchedPlans = snapshot.docs.map((doc) {
        return {
          'id': doc.id,
          'name': doc['name'] ?? '',
          'price': (doc['price'] as num).toDouble(), // Ensure price is a double
  ...
  Future<void> fetchUserOrders() async {
    try {
      ...
      CollectionReference subscriptionsRef = FirebaseFirestore.instance
          ...
          snapshot.docs.map((doc) async {
            ...
            return {
              ...
              'pickupLoc': locationData['pickup'] != null
                  ? LatLng(locationData['pickup'].latitude,
                  locationData['pickup'].longitude)
                  : null,
              'dropLoc': locationData['drop'] != null
                  ? LatLng(
                  locationData['drop'].latitude, locationData['drop'].longitude)
                  : null,
              'homePickupLoc': locationData['home']?['pickup'] != null
                  ? LatLng(locationData['home']['pickup'].latitude,
                  locationData['home']['pickup'].longitude)
                  : null,
              'homeDropLoc': locationData['home']?['drop'] != null
                  ? LatLng(locationData['home']['drop'].latitude,
                  locationData['home']['drop'].longitude)
                  : null,
              'workPickupLoc': locationData['work']?['pickup'] != null
                  ? LatLng(locationData['work']['pickup'].latitude,
                  locationData['work']['pickup'].longitude)
                  : null,
              'workDropLoc': locationData['work']?['drop'] != null
                  ? LatLng(locationData['work']['drop'].latitude,
                  locationData['work']['drop'].longitude)
                  : null,
              ...
              'deliveryDate': doc['deliveryDate'] != null
                  ? (doc['deliveryDate'] as Timestamp).toDate()
                  : null,
            };
          }).toList());
  ...
  Future<void> _geocodeAddress(String address,
    ...
    try {
      ...
      if (isPickupLocation) {
        ...
        markers.add(Marker(markerId: MarkerId('pickup'),
        ...
        markers.add(Marker(markerId: MarkerId('drop'),
  ...
  void handleDaySelection(String day) {
    ...
    if (currentDayIndex == today - 1) {
      showError(
          'We do not service on the current day. Please choose another day.');
      return;
    }
    ...
    setState(() {
      ...
      if (serviceType == 'Pickup every 2 days') {
        DateTime firstSelectedDay = DateTime.now().add(
            Duration(days: (currentDayIndex + 7 - today) % 7));
  ...
  void goToOrderSummary(BuildContext context) async {
    ...
    // Initialize the pickupLocation label for OrderSummaryScreen
    String pickupLocationLabel = 'Custom Location';
    if (pickupLocation == tempHomePickupLoc) {
      pickupLocationLabel = 'Home';
    } else if (pickupLocation == tempWorkPickupLoc) {
      pickupLocationLabel = 'Work';
    }

    // Calculate the end date (28 days from the pickup date)
    final DateTime endDate = selectedPickupDate!.add(const Duration(days: 28));

    // Navigate to OrderSummaryScreen, passing submitOrder with location type option
    final bool? isConfirmed = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => OrderSummaryScreen(
          ...
          onConfirmOrder: (String? locationType) => submitOrder(locationType),
        ),
      ),
    );

    if (isConfirmed == true) {
      fetchUserOrders(); // Refresh orders after successful submission
    } else {
        ...
        GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return GestureDetector(
              onTap: () =>
                  setState(() {
                    selectedPlan = plan['id']; // Store the plan ID
                    selectedPlanName = plan['name']; // Store the plan name
                    selectedPlanPrice =
                    plan['price']; // Store the plan price as double
                  }),
              child: Container(
                padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 8),
                decoration: BoxDecoration(
                  border: Border.all(
                      color: selectedPlan == plan['id'] ? Colors.blue : Colors
                          .grey),
                  borderRadius: BorderRadius.circular(10),
                  color: selectedPlan == plan['id'] ? Colors.blue[50] : Colors
                      .white,
                ),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    const SizedBox(height: 2),
                    const Text('A monthly service for 28 days',
                        style: TextStyle(
                ...
                child: Container(
                  ...
                  child: Center(
                    child: Text(
                      type,
                      style: TextStyle(
                        color: serviceType == type ? Colors.white : Colors
                            .black,
      ...
      children: [
        ...
        Wrap(
          ...
          children: daysOfWeek.skip(1).map((day) {
            ...
            DateTime currentDate = DateTime.now().add(
                Duration(days: (dayIndex + 7 - today) % 7));
              ...
              child: SizedBox(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(
                      color: isToday ? Colors.red : (isSelected
                          ? Colors.blue
                          : Colors.grey),
                      width: 2,
                    ),
        ...
        if (selectedPickupDate != null)
          Padding(
            ...
            child: Text(
              'Selected Pickup Date: ${DateFormat('MMMM dd, yyyy').format(
                  selectedPickupDate!)}',
        ...
        if (deliveryDate != null)
          Padding(
            padding: const EdgeInsets.only(top: 8.0),
            child: Text(
              'Estimated Delivery Date: ${DateFormat('MMMM dd, yyyy').format(
                  deliveryDate!)}',
              style: const TextStyle(
                  color: Colors.green, fontWeight: FontWeight.bold),
            ),
  ...
  Widget buildTimeSlotsSelection() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        const Text(
            'Choose 2 time slots (we recommend one in the morning and one in the evening):'),
  ...
  Future<bool> submitOrder(String? locationType) async {
    try {
      if (pickupLocation == null || dropLocation == null) {
        return Future.error('Please select both pickup and drop locations.');
      }

      User? user = FirebaseAuth.instance.currentUser;
      if (user == null) {
        return Future.error('No user is logged in. Please log in to place an order.');
      }
      String userId = user.uid;
      ...
      List<String> limitedSelectedDays = selectedDays.take(3).toList();
      ...
      Map<String, dynamic> orderData = {
        ...
        'serviceType': serviceType,
        'selectedDays': limitedSelectedDays,
        'timeSlots': timeSlots,
        'paymentDetails': {
          'amount': 100,
        ...
        'location': {
          ...
          'home': locationType == 'Home'
              ? {'pickup': GeoPoint(pickupLocation!.latitude, pickupLocation!.longitude), 'drop': GeoPoint(dropLocation!.latitude, dropLocation!.longitude)}
      ...
      return Future.error('Failed to submit order. Please try again.');
    }
  }
      ...
      onDragEnd: (newPos) {
        setState(() {
          ...
          mapController?.animateCamera(
              CameraUpdate.newLatLng(newPos)); // Move camera with marker
        });
      },
    ));
  }
            ...
            child: Column(
              ...
              children: [
                ...
                if (order['pickupLoc'] != null)
                  Text('Pickup Location: LatLng(${order['pickupLoc']
                      .latitude}, ${order['pickupLoc'].longitude})'),
                if (order['dropLoc'] != null)
                  Text('Drop Location: LatLng(${order['dropLoc']
                      .latitude}, ${order['dropLoc'].longitude})'),

                // Display home pickup and drop locations if available
                if (order['homePickupLoc'] != null)
                  Text('Home Pickup Location: LatLng(${order['homePickupLoc']
                      .latitude}, ${order['homePickupLoc'].longitude})'),
                if (order['homeDropLoc'] != null)
                  Text('Home Drop Location: LatLng(${order['homeDropLoc']
                      .latitude}, ${order['homeDropLoc'].longitude})'),

                // Display work pickup and drop locations if available
                if (order['workPickupLoc'] != null)
                  Text('Work Pickup Location: LatLng(${order['workPickupLoc']
                      .latitude}, ${order['workPickupLoc'].longitude})'),
                if (order['workDropLoc'] != null)
                  Text('Work Drop Location: LatLng(${order['workDropLoc']
                      .latitude}, ${order['workDropLoc'].longitude})'),

                // Show start date
                Text('Start Date: ${DateFormat('MMMM dd, yyyy').format(
                    order['startDate'])}'),

                // Show delivery date if it exists
                if (deliveryDate != null)
                  Text(
                    'Delivery Date: ${DateFormat('MMMM dd, yyyy').format(
                        deliveryDate)}',
                  ),

                // Show end date (stored in the database as 'endDate')
                Text('End Date: ${DateFormat('MMMM dd, yyyy').format(
                    order['endDate'])}'),

                Text('Amount Paid: ₹${order['paymentDetails']['amount']}'),

                // Display the selected days
                if (order['selectedDays'] != null &&
                    order['selectedDays'].isNotEmpty)
                  Text('Selected Days: ${order['selectedDays'].join(', ')}',
                      style: const TextStyle(fontSize: 16)),

                // Display the selected time slots
                if (order['timeSlots'] != null && order['timeSlots'].isNotEmpty)
                  Text('Time Slots: ${order['timeSlots'].join(', ')}',
                      style: const TextStyle(fontSize: 16)),

                if (order['planName'] != null)
                  Text('Current Plan: ${order['planName']}',
                      style: const TextStyle(fontSize: 16)),
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        Navigator.of(context).push(
                          MaterialPageRoute(
                            builder: (context) =>
                                EditOrderScreen(subscriptionId: order['id']),
                          ),
                        );
                      },
                    )
  ...
  Future<void> showPlanUpgradeDialog(String orderId) async {
    final currentOrder = userOrders.firstWhere((order) =>
    order['id'] == orderId);
                  ...
                  return ListTile(
                    ...
                    onTap: isCurrentPlan ? null : () {
                      ...
                      showConfirmUpgradeDialog(
                          orderId, planDoc.id); // Pass the selected plan ID
                    },
  ...
  Future<void> showConfirmUpgradeDialog(String orderId,
      String? selectedPlanId) async {
  ...
  Future<void> updateSubscriptionPlan(String orderId,
      String? selectedPlanId) async {
    try {
      DocumentReference planRef = FirebaseFirestore.instance.collection('plans')
          .doc(selectedPlanId);
      ...
      String? updatedPlanName = planSnapshot.exists
          ? planSnapshot['name']
          : 'No plan name';
      ...
      if (user != null) {
        await FirebaseFirestore.instance.collection('notifications').add({
          ...
          'userId': FirebaseFirestore.instance.collection('users').doc(
              user.uid),
        });
      }
    } catch (error) {
